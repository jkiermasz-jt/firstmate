...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the canonical local activity projection from `bin/fm-fleet-snapshot.sh --secondmate-home-summary`; retained `state/<id>.meta` records; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the away posture (`state/.afk-contract`, plus the `state/.afk` daemon flag where a daemon runs); and one cheap endpoint-presence read of each task's recorded backend endpoint. Registered secondmate activity remains owned by the cross-home `bin/fm-fleet-snapshot.sh --json` projection and is never inferred from retained parent records.
   The canonical projection owns current-state reads and runs them inside its own bounded snapshot call; if that projection is unavailable, the digest says so and never substitutes retained records or status events.  When you need a crew's actual current state for a run-step, read it with `bin/fm-crew-state.sh <id>` as before.
