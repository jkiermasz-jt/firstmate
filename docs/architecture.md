...
Only when no matching run exists does it consult semantic busy state; before that fallback, a recovery-grade tmux or Herdr endpoint read reconciles a readable shell-only endpoint, so stale progress is unknown while terminal or blocked events remain supervision-relevant. Exact busy reports working, and exact idle permits fallback to the log's resolved current declaration - the newest decision the fold still holds open, otherwise the latest recognized event - when its verb maps to a recognized run-state; unknown or a dead pane stays unknown instead of trusting a stale log.
